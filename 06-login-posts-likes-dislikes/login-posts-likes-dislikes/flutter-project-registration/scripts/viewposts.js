// 1. Постове (стена)
// ◦ Показвайте информация за регистрираният потребител
		// i. Име
		// ii. Брой постове
		// iii. Брой харесвания
// ◦ Добавяне на нов пост
		// i. При натискане на иконата в дясно от “Say something” трябва да се добави нов пост в базата данни
		// ii. Този пост трябва да има уникално id (използвайте генерираното от firebase)
		// iii. Запишете информация за потребителя който е добавил поста
		// iv. Увеличете броя на постовете за потребителя
// ◦ Показване на всички постове
		// i. На стената показвайте всички постове които съществуват в базата данни
		// ii. При поява на нов пост в Firebase – автоматично показвайте този пост за всички потребители
...
(() => {
	...
	firebase.auth().onAuthStateChanged(user => {
		...
		validateUser(user)
		const userRef = window.auth.getUserStats(user.uid);
		// userRef.once('value').then(userSnapshot => console.log(parseInt(userSnapshot.val()['likes']) + 1))
